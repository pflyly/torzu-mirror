...
std::unique_ptr<GraphicsPipeline> PipelineCache::CreateGraphicsPipeline(
    ...
    for (size_t index = uses_vertex_a && uses_vertex_b ? 1 : 0; index < Maxwell::MaxShaderProgram;
        ...
        const std::vector<u32> code{EmitSPIRV(profile, runtime_info, program, binding, Settings::values.optimize_spirv_output.GetValue())};
...
std::unique_ptr<ComputePipeline> PipelineCache::CreateComputePipeline(
    ...
    const std::vector<u32> code{EmitSPIRV(profile, program, Settings::values.optimize_spirv_output.GetValue())};
    device.SaveShader(code);
